...
void main() {
  group('AppDatabase insertOrReplace for sync pulled rows', () {
    ...
    test('inserting then re-inserting same clientUuid updates the row', () async {
    ...
    test('different clientUuids coexist (UNIQUE does not collide)', () async {
      await db
          .into(db.courses)
          .insert(CoursesCompanion.insert(clientUuid: 'course-a', title: 'A'));
      await db
          .into(db.courses)
          .insert(CoursesCompanion.insert(clientUuid: 'course-b', title: 'B'));
    ...
    test(
      'lessons / chapters / agenda also use insertOrReplace correctly',
      () async {
        // Lessons.
        final cid = 'course-1';
        await db
            .into(db.courses)
            .insert(CoursesCompanion.insert(clientUuid: cid, title: 'C'));
        await db
            .into(db.lessons)
            .insert(
        ...
        await db
            .into(db.agendaItems)
            .insert(
              AgendaItemsCompanion.insert(clientUuid: 'agenda-1', title: 'A1'),
            );
        await db
            .into(db.agendaItems)
            .insert(
              AgendaItemsCompanion.insert(
                ...
                title: 'A1',
                kind: const Value('revision'),
                subject: const Value('Algèbre'),
                ...
                chapterClientUuid: const Value('chapter-1'),
                pendingSync: const Value(false),
              ),
              mode: InsertMode.insertOrReplace,
            );
        ...
        expect(agenda.first.kind, 'revision');
        expect(agenda.first.subject, 'Algèbre');
        expect(agenda.first.location, 'Salle 204');
        expect(agenda.first.recurrence, 'weekly');
        expect(agenda.first.reminderMinutes, 10);
        expect(agenda.first.chapterClientUuid, 'chapter-1');
      },
    );

    test(
      'meta table insertOnConflictUpdate works on its primary key (cursor)',
      () async {
        ...
        final row = await (db.select(
          db.meta,
        )..where((m) => m.key.equals('courses'))).getSingle();
        expect(row.value, '5');
      },
    );

    test(
      'INSERT OR REPLACE works exactly as the sync engine does (regression)',
      () async {
